...
import 'speaking_p3_event.dart';
import 'speaking_p3_state.dart';

class SpeakingP3Bloc extends BaseBloc<SpeakingP3Event, SpeakingP3State> {
  final GetS3Questions getQuestionSpeakingP3;
  ...
  SpeakingP3Bloc({required this.getQuestionSpeakingP3})
    : super(SpeakingP3State()) {
  ...
  void _onSeekPlayback(SeekPlayback event, Emitter<SpeakingP3State> emit) {
    _audioPlayer.seek(event.position);
  }

  Future<void> _onStartRecording(
    StartRecording event,
    Emitter<SpeakingP3State> emit,
  ) async {
  ...
  Future<void> _onLoadQuestions(
    ...
    final result = await getQuestionSpeakingP3(
  ...
  void _onNextPart(NextPart event, Emitter<SpeakingP3State> emit) {
    if (state.currentIndex < state.listQuestion.length - 1) {
      emit(state.copyWith(currentIndex: state.currentIndex + 1));
    }
  }

  void _onPreviousPart(PreviousPart event, Emitter<SpeakingP3State> emit) {
    if (state.currentIndex > 0) {
      emit(state.copyWith(currentIndex: state.currentIndex - 1));
    }
  }

  void _onJumpToTopic(JumpToTopic event, Emitter<SpeakingP3State> emit) async {
